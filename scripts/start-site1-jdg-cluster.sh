...
for i in {1..3}; do 
    ...
    if [ "udp" == "@@jgroups.stack@@" ]; then 
        screen -d -m $DG_HOME/bin/clustered.sh -c jdg-site-1.xml \
            -P @@rhq.deploy.dir@@/properties/site1-jdg$i.properties \
            -u 239.1.1.1 
    else
        screen -d -m $DG_HOME/bin/clustered.sh -c jdg-site-1-tcp.xml \
            -P @@rhq.deploy.dir@@/properties/site1-jdg$i.properties \
